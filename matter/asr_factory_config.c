                                       ...
                                       uint32_t * puiOffset)
{
    ...
    do
    {
        ...
        ret = lega_flash_read(ASR_CONFIG_BASE, (uint32_t *) &usOffset, (void *) ucParaName, ASR_CONFIG_NAME_SIZE);
        if (ret != 0)
        {
            printf("lega_flash_read ASR_CONFIG_NAME_SIZE failed\r\n");
            return ret;
        }

        ret = lega_flash_read(ASR_CONFIG_BASE, (uint32_t *) &usOffset, (void *) &uiDataLen, ASR_CONFIG_LEN_SIZE);
        if (ret != 0)
        {
            printf("lega_flash_read ASR_CONFIG_LEN_SIZE failed\r\n");
            return ret;
        }
    ...
    *pBufLen = uiDataLen;
    ...
    ret = lega_flash_read(ASR_CONFIG_BASE, (uint32_t *) &usOffset, (void *) buf, uiDataLen);
...
int32_t asr_factory_config_write(uint8_t * configbuffer, uint32_t buf_len)
{
    ...
    return lega_flash_erase_write(ASR_CONFIG_BASE, (uint32_t *) &off_set, (void *) configbuffer, buf_len);
}
...
int32_t asr_factory_config_buffer_write(asr_matter_partition_t matter_partition, const void * buf, uint32_t buf_len)
{
    ...
    ret = lega_flash_read(ASR_CONFIG_BASE, (uint32_t *) &uiOffset, (void *) &uiDataLen, ASR_CONFIG_LEN_SIZE);
    if (ret != 0)
    {
    ...
    return lega_flash_erase_write(ASR_CONFIG_BASE, (uint32_t *) &uiOffset, (void *) buf, buf_len);
}
//extern int32_t lega_factory_decrypt_dac_prvkey(uint8_t * pDacCipher, uint8_t ucDacKeyLen, uint8_t * pRdBuf, uint32_t * pOutLen);
int32_t asr_factory_dac_prvkey_get(uint8_t * pRdBuf, uint32_t * pOutLen)
{
    ...
    // get DAC cipher
    ret = lega_flash_read(ASR_CONFIG_BASE, (uint32_t *) &off_set, (void *) ucCipher, 32);
    if (ret != 0)
        return ret;
    ...
    // if (ret != 0)
    // {
    //     ret = lega_factory_decrypt_dac_prvkey(ucCipher, 32, pRdBuf, pOutLen);
    //     if (ret != 0)
    //     {
    //         printf("lega_factory_decrypt_dac_prvkey fail\r\n");
    //         return ret;
    //     }
    // }
    // else
...
uint8_t asr_factory_check()
{
    ...
    ret = lega_flash_read(ASR_CONFIG_BASE, (uint32_t *) &uiOffset, ucRdBuf, uiRdLen);
    if (ret != 0)
    {
