 ...
 *
 * @param timer_in_sec    : input tmmt detect interval(second)
 *
 */
...
/** @brief  user use to config sleep type
 *
 * @param sleep    : sleep type, refer to lega_sleep_type_e
 * @param sleep    : sleep config, refer to lega_sleep_config_t, only used for deep sleep, others
 *                   set to NULL
 *
 * @return     0   : on success.
 * @return    -1   : config error
 * @return    -2   : sleep macro is not enabled
 */
int lega_wlan_goto_sleep(lega_sleep_type_e sleep, lega_sleep_config_t *config);

#ifdef LIGHT_SLEEP_SUPPORT
/** @brief  user use to config sleep type to be light sleep and soc off, should be called before lega_wlan_set_ps_mode()
 *
 * @param sleep    : sleep config, refer to lega_sleep_config_t, only used for deep sleep(to be updated
 *                   for light sleep and soc off), others set to NULL
 *
 * @return     0   : on success.
 * @return    -1   : config error
 * @return    -2   : sleep macro is not enabled
 */
int lega_wlan_goto_lightsleep_socoff(lega_sleep_config_t *config);
#endif
