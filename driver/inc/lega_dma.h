...
typedef void (*lega_dma_callback_func)(void);
...
typedef struct {
    uint32_t     src;    //src
    uint32_t     dest;    //dst
    uint8_t      data_width; //0:8bit,1:16bit,2:32bit
    uint8_t      src_msize; //0:1 width,1:4 width,2:8 width,x:2*((x-1)'s width)
    uint8_t      dest_msize; //0:1 width,1:4 width,2:8 width,x:2*((x-1)'s width)
    uint16_t     block_size; //data size = block_size * data_width
    uint8_t      ch;     //channel
