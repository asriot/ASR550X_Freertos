...
struct LEGA_RETENTION_SRAM
{
    ...
    uint8_t RSVD1[40];
#ifdef QUICK_CONNECT
    uint32_t RF10_CAL[8+8+16+16+8+8+1];
    int32_t RF11_CAL[14*2+1];
    uint32_t WIFI_CFG[384/4];
    uint32_t USER_PMK[36/4];
#endif
};
#define RETENTION_RAM_ADDR              0x40008000
#define RETENTION_SRAM                  ((struct LEGA_RETENTION_SRAM *)(RETENTION_RAM_ADDR))

#define LEGA_RETENTION_SRAM_CUSTOM_SIZE 64
...
#define uart_put_char_2(c) do{\
    if(g_printf2_uart == 0) \
    {\
        ((UART0)->THR) = (c); \
        while((((UART0)->LSR) & DW_UART_TX_TC) == 0);\
    }\
    else if(g_printf2_uart == 1) \
    {\
        ((UART1)->THR) = (c); \
        while((((UART1)->LSR) & DW_UART_TX_TC) == 0);\
    }\
    else if(g_printf2_uart == 2) \
    {\
        ((UART2)->THR) = (c); \
        while((((UART2)->LSR) & DW_UART_TX_TC) == 0);\
    }\
...
#define uart_put_char(c) do{\
    if(g_printf_uart == 0) \
    {\
        while(!((UART0)->USR & (DW_UART_USR_TFNF))); \
        ((UART0)->THR) = (c); \
    }\
    else if(g_printf_uart == 1) \
    {\
        while(!((UART1)->USR & (DW_UART_USR_TFNF))); \
        ((UART1)->THR) = (c); \
    }\
    else if(g_printf_uart == 2) \
    {\
        while(!((UART2)->USR & (DW_UART_USR_TFNF))); \
        ((UART2)->THR) = (c); \
    }\
}while(0)
