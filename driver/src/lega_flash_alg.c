...
FLASH_DRIVER_SEG int lega_flash_alg_cache_bypass_ex(void)
{
    //cache bypass
    if(!(FLASH_CACHE_CTRL & CACHE_BYPASS))
    {
...
vu32 lega_flash_line_cfg = 0; //flash line number config
FLASH_DRIVER_SEG void lega_flash_alg_load_cfg () {
    ...
    FLASH->QSPI_CCR   = lega_flash_line_cfg;   //dumode=2'b10,,sio=1'b0,fmode=2'b11,dmode=2'b10,reserved=1'b0,dcyc=5'h7,absize=2'b00,abmode=2'b00,adsize=2'b10,admode=2'b01,imode=2'b01,instruction=8'h3B;
...
FLASH_DRIVER_SEG int lega_flash_alg_init ()
{
    if(g_chip_is_smic)
    {
        int bypassed = lega_flash_alg_cache_bypass_ex();

        lega_flash_line_cfg = FLASH_QSPI_L4; //config flash line here

        if(lega_flash_line_cfg == FLASH_QSPI_L4)
        {
            lega_flash_alg_setqe(1);
        }
        else
        {
            lega_flash_alg_setqe(0);
        }
...
#ifdef CFG_RFOTA_RX
        lega_flash_line_cfg = FLASH_QSPI_L2; //config flash line here
#else
        lega_flash_line_cfg = FLASH_QSPI_L4; //config flash line here
#endif
...
FLASH_DRIVER_SEG int lega_flash_alg_erase(unsigned int cmd, unsigned long adr) {
    ...
    if(g_chip_is_smic)
    {
        bypassed = lega_flash_alg_cache_bypass_ex();
    }
