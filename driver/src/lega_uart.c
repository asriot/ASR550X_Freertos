...
{
    while(!(UARTx->USR & DW_UART_USR_TFNF));
    UARTx->THR = Data;
...
/**
 * Check UART interface dev valid
 *
 * @param[in] uart interface dev
 * @return 0: success, -1: fail
 */
int32_t lega_uart_dev_check_valid(lega_uart_dev_t *uart)
{
    if(NULL == uart)
    {
        return -1;
    ...
    if(    LEGA_UART0_INDEX != uart->port
        && LEGA_UART1_INDEX != uart->port
        && LEGA_UART2_INDEX != uart->port)
    return -1;
...
int32_t lega_uart_init(lega_uart_dev_t *uart)
{
    ...
    if(lega_uart_dev_check_valid(uart)<0)
    {
        return -1;
    }
    ...
    {
        volatile uint32_t vol_temp = UARTx->USR;    //clear some unkown irqs, for #3067
        (void)vol_temp;
    }
...
void lega_uart_set_callback(uint8_t uart_idx,lega_uart_callback_func func)
{
    g_lega_uart_callback_handler[uart_idx] = func;
}