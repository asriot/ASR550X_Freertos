...
uint32_t peri_addr[8] = {(uint32_t)UART0,(uint32_t)UART1,(uint32_t)UART2,SPIx_DR(0, 0),SPIx_DR(1, 0),SPIx_DR(2, 0),I2C0_BASE,I2C1_BASE};
//uint8_t buff_p2m = 0;
...
{
    uint8_t dma_ch = 0;
    if(REG_RD(DMA_STATUS_BLOCK_L_REG)&0x01)
    {
        dma_ch = 0;
    }
    else
    ...
    {
        g_lega_dma_callback_handler[dma_ch]();
    }
    if(dma_ch == 0)
    {
        //clear TFR int,channelx
        REG_WR(DMA_CLEAR_TFR_L_REG,0x1);
        //clear block int,channelx
        REG_WR(DMA_CLEAR_BLOCK_L_REG,0x1);
    }
    else
    {
       //clear TFR int,channelx
        REG_WR(DMA_CLEAR_TFR_L_REG,0x2);
        //clear block int,channelx
        REG_WR(DMA_CLEAR_BLOCK_L_REG,0x2);
    }
}
...
//        *((volatile int *)config_reg.DMA_CTL_H) = dma->block_size;
    ...
    *((volatile int *)config_reg.DMA_CTL_H) = dma->block_size;
    //[16:14]=000, src_size
    lega_write32_bit(config_reg.DMA_CTL_L_REG,14,3,dma->src_msize);
    //[13:11]=000, dest_size
    lega_write32_bit(config_reg.DMA_CTL_L_REG,11,3,dma->dest_msize);
    //[6:4]=000,   source width
    lega_write32_bit(config_reg.DMA_CTL_L_REG,4,3,dma->data_width);
    //[3:1]=000,   destination width
    lega_write32_bit(config_reg.DMA_CTL_L_REG,1,3,dma->data_width);
    //[0]=1,       int enable
...
/**
 * update src dest addr
 *
 * @param[in]  dma  the dma interface
 * @param[in]  size  number of bytes to transmit/recive
 *
 * @return
 */
void lega_dma_addr_update(lega_dma_dev_t *dma,uint32_t size)
{
    if(dma->ch == 0)
    {
        while(CHENREG_L&0x01);
 ...
 * dma channel en
 *
* @param[in]  dma  ch num 0:ch0,1:ch1,other both
 *
 * @return
 */
void lega_dma_ch_enable(uint8_t ch)
{
//    uint32_t temp_val = 0;
    if(!(DMACFGREG_L & 0x1))
    {
    ...
    {
        while((CHENREG_L&0x02) || (CHENREG_L&0x01));
        CHENREG_L = 0x0303;
    }
}
 ...
 * @return
 */
void lega_dma_ch_disable(uint8_t ch)
{
//    if(!(DMACFGREG_L & 0x1))
//    {
//        DMACFGREG_L = 0x1;
